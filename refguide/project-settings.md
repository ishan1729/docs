...
When this option is enabled, then the App will perform optimistic locking during update and delete. This prevents overwriting changes to data made concurrently by another user. When this option is disabled, then the behavior in case of a concurrent modification is that the last writer wins. This is the behavior of Mendix 7.4 and earlier.
